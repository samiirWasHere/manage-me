...
const BoardCardForm = (props) => {
  ...
  const handleClick = useCallback((fetchData, updatedData) => {
    return fetchData.filter((item) => item.isValid !== updatedData.)
  });

  const removeDuplicatesItems = () => {
    let errorObj = {
      titleError: '',
      descriptionError: '',
      dateError: new Date()
    ...
    if(!title){
      errorObj.titleError = "This field is required"
    }

    if(!description){
      errorObj.descriptionError = "This field is required"
    }

    if(!date){
      errorObj.dateError = "This field is required"
    }

    return errorObj.titleError && errorObj.descriptionError && errorObj.dateError
  }
